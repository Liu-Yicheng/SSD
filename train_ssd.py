    ...
    with tf.device('/cpu:0'):
        with tf.control_dependencies([cls_pred, location_pred]):
            with tf.name_scope('post_forward'):
                ...
                flaten_cls_targets = tf.reshape(cls_targets, [-1])#[batch_size*8732]
                flaten_match_scores = tf.reshape(match_scores, [-1])
                flaten_loc_targets = tf.reshape(loc_targets, [-1, 4])#[batch_size*8732, 4]
...
def main(_):
    ...
    #每个gpu应该拿出多少的资源给进程，0.7代表70%
    gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=FLAGS.gpu_memory_fraction)
    #1.allow_soft_placement:允许tf自动选择一个存在并且可用的设备来运行操作
    #2.log_device_placement:可以获取到 operations 和 Tensor 被指派到哪个设备(几号CPU或几号GPU)上运行
                           #会在终端打印出各项操作是在哪个设备上运行的
    #3.通过设置intra_op_parallelism_threads参数和inter_op_parallelism_threads参数，
       #来控制每个操作符op并行计算的线程个数。二者的区别在于:
       #3.1 intra_op_parallelism_threads 控制运算符op内部的并行,当运算符op为单一运算符，并且内部可以实现并行时
           #如矩阵乘法，reduce_sum之类的操作，可以通过设置intra_op_parallelism_threads参数来并行
       #3.2 inter_op_parallelism_threads 控制多个运算符op之间的并行计算
    ...
    #当使用多GPU时，batch_size应该是GPU个数的倍数
