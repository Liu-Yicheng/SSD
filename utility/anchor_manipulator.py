...
def iou_matrix(gt_bboxes, default_bboxes):
    #gt_bboxes:prior_bbox, default_bboxes:gt_bbox
...
def do_dual_max_match(overlap_matrix, low_thres, high_thres, ignore_between=True, gt_max_first=True):
    ...
    Args:
        overlap_matrix: num_anchors * num_gt  [8732, m]
    '''
    with tf.name_scope('dual_max_match', [overlap_matrix]):
        # first match from anchors' side
        # anchors_to_gt:[8732],第i位数字x 代表第i个prior_bbox与第x个gt框的IOU最大
        anchors_to_gt = tf.argmax(overlap_matrix, axis=1)
        # match_values:[8732] 第i位数字x 代表第i个prior_bbox与所有gt框最大的IOU
        ...
        # match_indices:[8732], 第i个数字如果为-1， 表示第i个prior_bbox与所有的gt框的最大IOU<low_thres
        #                       第i个数字如果为-2， 表示第i个prior_bbox与所有的gt框的最大IOU属于[low_thres, high_thres]
        #                       出现任何>0的数字x， 表示第i个prior_bbox与第x号的gt框的IOU最大
        ...
        # anchors_to_gt_mask:[8732, m],其中第二维度是one-hot编码
        ...
        #gt_to_anchors_overlap: [1,m],第i个数字表示：第i个gt框与所有的prior_bbox的最大IOU
        gt_to_anchors_overlap = tf.reduce_max(overlap_matrix, axis=0, keepdims=True)

        #left_gt_to_anchors_mask:[8732, m], 每一列是one-hot,
        ...
        #seected_scores后面tf.stack部分:[8732, 2]。[i，j]代表第i个prior_bbox，与第j个GT框达成勾搭关系
        #selected_score:[8732] 第i个数字x表示，第i个priorbbox与所以GT框的最大IOU是x
        ...
        #第一个返回值：[8732],其中第i个数字x 表明它与第x个gt框达成勾搭关系，-1则表示它与gt最大的IOU<low_thresh, -2表示它与gt最大的IOU在low和hign之间
        #第二个返回值：[8732],其中第i个数字表明，它与gt框的最大IOU
...
class AnchorEncoder(object):
    #positive_threshold=0.5, ignore_threshold=0.5, prioi_scaling:[0.1, 0.1, 0.2, 0.2]
    ...
    def generate_anchors_by_offset(self, anchors_height, anchors_width, anchor_depth, image_shape, layer_shape, feat_stride, offset=0.5, name=None):
        ...
        with tf.name_scope(name, 'generate_anchors'):
            ...
            if isinstance(offset, list):
                ...
                offset_h = offset
                offset_w = offset
            #x_on_image: 图片分成32*32个格子， 每个格子的中心点x在图片上的坐标 [32, 32]
            #x_on_image: 图片分成32*32个格子， 每个格子的中心点y在图片上的坐标 [32, 32]
    ...
    def get_all_anchors(self, image_shape, anchors_height, anchors_width, anchors_depth, anchors_offsets, layer_shapes, feat_strides, allowed_borders, should_clips, name=None):
        ...
        #anchors_height [[],[],[],[],[],[]]
        #anchors_weight [[],[],[],[],[],[]]
        #anchors_depth [4, 6, 6, 6, 4, 4]
        #anchors_offsets:[0.5, 0.5, 0.5, 0.5, 0.5, 0.5]
        #layer_shapes [(38, 38), (19, 19), (10, 10), (5, 5), (3, 3), (1, 1)]*[0.5]
        #feat_strides [8, 16, 32, 64, 100, 300]
        #allowed_border:[300,300,300,300,300,300]
        #should_clips:[FALSE,FALSE,FALSE,FALSE,FALSE,FALSE]
        with tf.name_scope(name, 'get_all_anchors'):
            ...
            for ind, anchor_depth in enumerate(anchors_depth):
                with tf.name_scope('generate_anchors_{}'.format(ind)):
                    #_anchors_ymin:[layer_shapes[ind]*layer_shapes[ind], 6]
            ...
            #anchors_ymin:[4*38*38+6*19*19+6*10*10+6*5*5+4*3*3+4*1*1)]
    ...
    def encode_anchors(self, labels, bboxes, anchors_ymin, anchors_xmin, anchors_ymax, anchors_xmax, inside_mask, debug=False):
        ...
        '''
        # anchors_ymin:[4*38*38+6*19*19+6*10*10+6*5*5+4*3*3+4*1*1)]=>[8732]
        with tf.name_scope('encode_anchors'):
            # all_anchors:[8732, 4] inside_mask:[8732]:若第i个prior_bbox有超出原图的，第i个数字为0
            all_anchors = tf.stack([anchors_ymin, anchors_xmin, anchors_ymax, anchors_xmax], axis=-1)

            #overlap_matrix:[8732, 4]
            ...
            # 经过上面3步 gt_labels:[8732], 勾搭上的prior_bbox有着和它勾搭的gt框一样的label值， 其余没有勾搭上的为 -1
