...
def ssd_random_sample_patch(image, labels, bboxes, ratio_list=[0.1, 0.3, 0.5, 0.7, 0.9, 1.], name=None):
  ...
  def sample_width_height(width, height):
    ...
    # while((sampled_width > sampled_height*2) or (sampled_height > sampled_width*2) and index <50) or index < 1):
    ...
    #   index += 1
    # 当结束循环时，返回的sampled_width，sampled_height有如下关系：sampled_width < sampled_height*2
    #                                                       sampled_height < sampled_width*2
  ...
  def check_roi_center(width, height, labels, bboxes):
    with tf.name_scope('check_roi_center'):
      ...
      """
      while(np.reduce_sum(mask)<1 and index < 50) or index < 1:
        #返回的sampled_width，sampled_height有如下关系：sampled_width < sampled_height*2 ，sampled_height < sampled_width*2
      ...
      循环的整体逻辑是 ：
            step1.产生满足条件的roi
            step2.判断该roi内是否有bbox的中心点
            step3,若有则推出循环返回该roi，mask_label, mask_bbox 若无则跳回第一步
      """
      def condition(index, roi, mask):
        #循环执行条件：1.循环没有执行过，2.mask中全为0并且执行次数小于50
  ...
  def check_roi_overlap(width, height, labels, bboxes, min_iou):
    with tf.name_scope('check_roi_overlap'):
      ...
      mask_bboxes = bboxes
      """
      while(np.reduce_sum((jaccard_with_anchors(roi, mask_bbox)< min_iou)) < 1 and index < 50) or index<1 or mask_label[0]<1
           roi, mask_labels, mask_bboxes = check_roi_center(width, height, labels, bboxes)
      该循环的整个逻辑是：
        step1. 循环直到产生一个符合边长条件的ROI
        step2. 循环 判断该ROI内是否有gt框中心点，若无则返回step1。若有循环中断产生roi, mask_labels, mask_bboxes
        step3. 判断ROI是否至少与一个mask_bboxes中的box的IOU>min_iou,若有中断循环，若没有返回第一步。
        step4. 当每个循环都尝试了max_ttemp次，没有找到合适的ROI， 返回原图大小及原来的labels和boxes
               当找到了合适的ROI，则返回ROI及中心点在ROI内部的gt框
  ...
  with tf.name_scope('ssd_random_sample_patch'):
    ...
    #tf.multinomial(tf.log([[1/6, 1/6, 1/6, 1/6, 1/6, 1/6]]), 1) 取index 0-6的概率都一样，按概率随机取一个=>[[2]]
...
def ssd_random_expand(image, bboxes, ratio=2., name=None):
  #这个函数主要是给图片长宽都增加一倍，左边右边增加的和为原图片大小，比例任意
  with tf.name_scope('ssd_random_expand'):
    ...
    #float_hegith, float_width 是图片的原始大小
    #canvas_width, canvas_height 是填充后的图片大小，因为ratio=2 所以canvas_width=2*float_width
    #canvas_width - width 是总共宽增加的长度w_, x左边填充长度属于[0, w_],x_右边填充长度=w_-x
